...
const api_key = "api_key=714faf6b3e66506f6a89dc12773eba9e";
...
const high_api =
  base_url +
  "/discover/tv/?certification_country=US&certification=R&sort_by=vote_average.desc&" +
  api_key;
const kids_api =
  base_url +
...
const best_api =
  base_url +
  "/discover/tv?primary_release_year=2020&sort_by=vote_average.desc&" +
  api_key;
const fav_api =
  base_url +
  "/discover/tv?with_genres=18&primary_release_year=201&" +
  api_key;
...
function searchMovie(searchTerm) {
  const url = `${base_url}/search/tv?${api_key}&query=${searchTerm}`;
  fetch(url)
    .then((res) => res.json())
    .then((data) => {
      console.log(data);
...
function showSearchedmovies(data) {
  ...
  data.forEach((movie) => {
    const {name, poster_path, release_date} = movie;
